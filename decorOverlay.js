...
import { createVillageCharacters, createForestCharacters, createFieldFarmers } from './characterOverlay.js';
...
const        FOUNTAIN_1_TARGET_WIDTH         = HEX_SIZE * 0.18 * 0.93 * 0.90 * 0.96 * 0.91 * 0.93 * 0.87; // −7% −10% −4% −9% −7% −13% (2026-07-04)
const        FOUNTAIN_2_TARGET_WIDTH         = HEX_SIZE * 0.18 * 0.93 * 0.80 * 0.96 * 0.91 * 0.93 * 0.87; // −7% −20% −4% −9% −7% −13% (2026-07-04)
...
const PROP_MODEL_DEFS = [
  ...
  // Personnages — villageois et rôdeurs de forêt (GLB individuels, /glb/characters/).
  // Placés par characterOverlay.js (createVillageCharacters / createForestCharacters).
...
function _rebuildRoadsideDecorLOD(overlay) {
  ...
  const _characterDistSq = LOD_CHARACTER_CULL_DISTANCE    * LOD_CHARACTER_CULL_DISTANCE;
  ...
  for (const subGroup of overlay.children) {
    ...
    } else if (subGroup.name === 'village-characters-glb' || subGroup.name === 'forest-characters-glb' || subGroup.name === 'field-farmer-characters-glb') {
      for (const child of subGroup.children) {
        overlay.userData.roadsideDecorObjects.push({ object: child, center: child.position.clone(), lodDistSq: _characterDistSq });
      }
    }
  }
}
...
export function rebuildDecorOverlay(overlay, placedTiles) {
  ...
  overlay.add(createVillageCharacters(placedTiles, specialBuildingSafeZones));
  overlay.add(createForestCharacters(placedTiles, specialBuildingSafeZones));
  overlay.add(createFieldFarmers(placedTiles, specialBuildingSafeZones));
...
export function addSingleTileToDecorOverlay(overlay, newPlacedTile, placedTiles) {
  ...
  // ── Personnages village : delta dans le groupe existant ───────────────────
  const villageCharGroup = overlay.getObjectByName('village-characters-glb');
  if (villageCharGroup) {
    const newVillageChars = createVillageCharacters(singleTileMap, specialBuildingSafeZones);
    while (newVillageChars.children.length > 0) villageCharGroup.add(newVillageChars.children[0]);
  }

  // ── Personnages forêt : delta dans le groupe existant ─────────────────────
  const forestCharGroup = overlay.getObjectByName('forest-characters-glb');
  if (forestCharGroup) {
    const newForestChars = createForestCharacters(singleTileMap, specialBuildingSafeZones);
    while (newForestChars.children.length > 0) forestCharGroup.add(newForestChars.children[0]);
  }

  // ── Fermiers des champs de blé : delta dans le groupe existant ────────────
  const fieldFarmerGroup = overlay.getObjectByName('field-farmer-characters-glb');
  if (fieldFarmerGroup) {
    const newFieldFarmers = createFieldFarmers(singleTileMap, specialBuildingSafeZones);
    while (newFieldFarmers.children.length > 0) fieldFarmerGroup.add(newFieldFarmers.children[0]);
  }
