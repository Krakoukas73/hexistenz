...
- **Convention adoptée côté jeu** (différente de `tr()` PHP) : chaque module JS fait son propre `await fetch('./json/languages/${lang}.json')` top-level (précédent déjà en place : `edaPanelWiring.js`/`ambiances.json`), lit `localStorage['hexistenz_pres_lang']` (repli `'fr'`), avec repli textuel FR codé en dur à chaque site d'usage (`?? 'texte français'`) si le fetch échoue. Chaque fichier reste autonome (philosophie "pas de bundler" du projet) ; les fetches dupliqués du même petit JSON sont sans conséquence (cache HTTP navigateur). **⚠️ Périmé le 2026-07-13** : le texte était figé une fois pour toutes au chargement de chaque module (pas de re-fetch/re-render en cours de partie) — insuffisant une fois le sélecteur de langue in-game ajouté (cf. entrée 2026-07-13 ci-dessous, `gameLangReactive.js`), qui rend ces mêmes textes réactifs sans réintroduire de loader central.
...
**🪤 Incident — ajout de l'espagnol (ES), corruption réelle de fichiers, rollback complet (2026-07-12/13)** : tentative d'étendre le bilingue FR/EN à une 3e langue (ES) sur la prez ET le jeu, même mécanisme que l'entrée précédente. Des scripts bash (Python, lecture-puis-réécriture sur le point de montage réseau `X:\...`) ont réellement tronqué 3 fichiers (`scene.js`, `edaPanelWiring.js`, `edaPanelHost.js`) — pas un problème d'affichage, une perte de données sur disque (cf. mémoire `feedback-verif-mount-cowork`, mise à jour suite à cet incident : **ne plus jamais faire de lecture-puis-réécriture bash sur ce mount**, utiliser exclusivement les outils Read/Edit/Write). Une reconstruction a été tentée depuis l'historique de conversation et jugée structurellement correcte après relecture complète, mais le jeu restait cassé en pratique (tuiles posées invisibles après lancement d'une partie, aucune erreur console) — cause jamais identifiée malgré un audit exhaustif de toute la chaîne de placement/rendu. L'utilisateur a choisi de restaurer un backup complet plutôt que de poursuivre le diagnostic (2026-07-13). **État actuel : l'espagnol N'EST PAS présent** dans le jeu ni la prez — ne pas supposer son existence tant qu'il n'a pas été explicitement réintroduit et validé en jeu réel (pas seulement vérifié syntaxiquement).
