...
**📋 PASSATION — Merge Piregwan↔Cyril, branche `merge/piregwan-2026-07-06`** (2026-07-07, par Claude/Cyril) : le delta Piregwan (build v0.9.2, sync 05→06/07 : fix curseurs fantômes ci-dessus, `gpuProfiler.js`, `hud_fps.js`, `tileTextures.js`, `tileUtils.js`, `waterZoneOverlay.js`) a été fusionné avec les optims/VFX Cyril (perf steps 1-7, densité de contenu, VFX Phase 0/1a) sur **une branche séparée, poussée sur origin, `main` non touchée** : `origin/merge/piregwan-2026-07-06` (commit `f6e3153`, base commune `386dad4`). 3 fichiers en conflit réel résolus best-of-both (`scene.js`, `threeSetup.js`, `variables.js` — détail dans le message de commit). `waterDebugUi.js` supprimé (code mort des deux côtés, fusionné dans l'EDA). Vérifié end-to-end (`node --check`, `php -l`, lancement `game.php` → PLATISTE) sans erreur console.

**⚠️ Point laissé à l'arbitrage de Piregwan** : les fonctions de diagnostic qu'il a ajoutées dans `scene.js` et appelées **chaque frame** (`warmUpAllPrograms('perframe')`, `checkProgramChurn()`, `checkBiomeMaterialFlicker()`, `findTransparentBiomeUsers()`, log `[RAF-STALL]`) sont un reliquat de l'investigation du throttle GPU **déjà résolue** juste au-dessus. Elles ont été conservées telles quelles dans le merge (fidèles à son intention, zéro régression) mais tournent en continu et vont à l'encontre des optims perf (coût CPU non mesuré par le merge). À évaluer : les couper par défaut derrière un flag, ou les retirer si Piregwan confirme qu'elles ne servent plus.

**Workflow retenu (2026-07-07)** : pas de mise en place d'un dossier de travail git+symlinks partagé pour Piregwan — il continue de bosser depuis son dossier Google Drive et Cyril lui envoie ses évolutions (VFX à venir) sous forme de **zips**, à charge pour Piregwan de les intégrer lui-même à son rythme.
