...
export function isSingleTerrainFootprint(local, placedTile, expectedType, radius) {
  // Garde centre-eau : si le centre de la tuile est eau, tout point dans la zone centrale
  // est invalide, même si son angle tombe dans un secteur d'arête valide.
  // centerRadiusScale = 0.33 + marge turbulence ragged-edges → 0.38.
  const CENTER_WATER_GUARD = HEX_SIZE * 0.38;
  const isCenterWater = getTileCenterType(placedTile) === EDGE_TYPES.water;
  ...
  for (const sample of samples) {
    // Rejet si dans la zone centrale eau (angle-only serait trompeur ici)
    if (isCenterWater && Math.hypot(sample.x, sample.z) < CENTER_WATER_GUARD) return false;
