...
export const HEX_FONT_FAMILY = 'BebasNeue, system-ui, sans-serif';
...
const _face = new FontFace('BebasNeue', "url('./fonts/BebasNeue-Regular.ttf')", {
  weight: '100 900',
  display: 'block',
});
...
export const hexFontReady = _face.load()
  .then(f => {
    ...
    return document.fonts.load('900 96px BebasNeue');
  })
  .then(() => {
    sharedLabelCache.clear();
  })
  .catch(err => {
    console.error('[BebasNeue] ✗ échec chargement :', err);
    sharedLabelCache.clear();
  });
