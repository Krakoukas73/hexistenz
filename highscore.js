import { LUT_HELP, ensureHelpTooltip, delegateHelpTooltip } from './help.js';
...
export function createHighscoreUI(ui) {
  ...
  // Tooltip élégant au survol des chips de stats (délégation — innerHTML rebuilt)
  ensureHelpTooltip();
  const listContainer = elements.list?.parentElement;
  if (listContainer) delegateHelpTooltip(listContainer, 'hs-help', LUT_HELP);
...
function renderCompactStats(stats) {
  ...
  const textureChips = STAT_TYPES
    .map(([type, emoji, className]) => {
      ...
      return `<span class="highscore-stat-chip ${className}" data-hs-help="hs.${type}">${emoji}${total}/${largest}</span>`;
    })
    .join('');
  ...
  const summary = `<span class="highscore-stat-chip stats-summary-chip" data-hs-help="hs.summary">⬢${tiles} 🚂${trains} ⛵${boats} ☄️${comets}</span>`;
